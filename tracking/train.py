...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str,  default='mfja', help='training script name')
...
def main():
    ...
    if args.mode == "single":
        ...
        train_cmd = "python -m torch.distributed.launch --nproc_per_node %d --master_addr %s --master_port %d --nnodes %d "\
                    "--node_rank %d lib/train/run_training.py " \
                    "--script %s --config %s --save_dir %s --use_lmdb %d --script_prv %s --config_prv %s --use_wandb %d " \
                    "--distill %d --script_teacher %s --config_teacher %s" \
