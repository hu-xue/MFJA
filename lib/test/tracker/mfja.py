...
class MFJATrack(BaseTracker):
    '''MFJATrack tracker'''
    def __init__(self, params):
        super(MFJATrack, self).__init__(params)
        network = build_mfjatrack(params.cfg, training=False)  # build the mfja network
        network.load_state_dict(torch.load(self.params.checkpoint, map_location='cpu')['net'], strict=True)    # load the checkpoint
        self.cfg = params.cfg
        self.network = network.cuda()  # move the network to GPU
