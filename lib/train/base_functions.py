...
def build_dataloaders(cfg, settings):
    ...
    loader_train = LTRLoader('train', dataset_train, training=True, batch_size=cfg.TRAIN.BATCH_SIZE, shuffle=shuffle,
                             num_workers=cfg.TRAIN.NUM_WORKER, drop_last=True, stack_dim=1, sampler=train_sampler)  # 加载训练集
    ...
    if cfg.DATA.VAL.DATASETS_NAME[0] is None:
        ...
        dataset_val = sampler.TrackingSampler(datasets=names2datasets(cfg.DATA.VAL.DATASETS_NAME, settings, opencv_loader),
                                            p_datasets=cfg.DATA.VAL.DATASETS_RATIO,  # 采样频率
        ...
        loader_val = LTRLoader('val', dataset_val, training=False, batch_size=cfg.TRAIN.BATCH_SIZE,
                            ...
                            epoch_interval=cfg.TRAIN.VAL_EPOCH_INTERVAL)  # 加载验证集

    return loader_train, loader_val


def get_optimizer_scheduler(net, cfg):
    '''冻结不微调的参数，为需要微调的参数设置优化器和配置学习率和学习率衰减策略'''
    train_type = getattr(cfg.TRAIN.PROMPT, "TYPE", "")
    
    if 'mfja' in train_type:
        print("Only training et parameters. They are: ")
        param_dicts = [{"params": [p for n, p in net.named_parameters() if ("adap" in n or "prompt" in n) and p.requires_grad]}]
        for n, p in net.named_parameters():
            if "prompt" not in n and "adap" not in n:
        ...
        print('=Total: ',total_num*1e-6, 'Trainable: ',trainable_num*1e-6)
    ...
    if cfg.TRAIN.OPTIMIZER == "ADAMW":
        optimizer = torch.optim.AdamW(param_dicts, lr=cfg.TRAIN.LR,
                                      weight_decay=cfg.TRAIN.WEIGHT_DECAY)  # 优化需要更新的参数
    else:
