...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
        ...
        """
        self.datasets = datasets  # 数据集列表
        self.train_cls = train_cls  # whether we are training classification
        self.pos_prob = pos_prob  # probability of sampling positive class when making classification, 分类时采样正类的概率

        # If p not provided, sample uniformly from all videos, 如果没有提供 p，则从所有视频中均匀采样
        if p_datasets is None:
            p_datasets = [len(d) for d in self.datasets]

        # Normalize
        p_total = sum(p_datasets)
        self.p_datasets = [x / p_total for x in p_datasets]  # 数据集权重列表
