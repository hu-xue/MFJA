...
# todo
class MFJATActor(BaseActor):
    """ Actor for training MFJA models """
    ...
    def __call__(self, data):
        ...
        """
        # forward pass
        out_dict = self.forward_pass(data)  # 返回模型的输出结果

        # compute losses
        loss, status = self.compute_losses(out_dict, data)  # 根据模型的输出结果和输入数据计算loss
