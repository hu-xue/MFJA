...
from lib.models.layers.adapter import Fusion_adapter, Iner_adapter
from lib.models.layers.prompt import Bi_prompt
...
class CEABlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, keep_ratio_search=1.0, layer_num=None):
        super().__init__()
    ...
    def forward(self, x, xi, global_index_template, global_index_templatei, global_index_search, global_index_searchi, \
        ...
        x_norm = self.norm1(x)
        xi_norm = self.norm1(xi)
        
        x_attn, attn = self.attn(x_norm, mask, True)  # norm + MHA
        if self.layer_num in [4,7,10]:
            x_xi_norm = torch.cat([x_norm, xi_norm], dim=-1)
            x_xi_fusion = self.adap_t(x_xi_norm)
            x_fusion, xi_fusion = torch.split(x_xi_fusion, x_norm.size(-1), dim=-1)
            x_adap = self.iner_adap1(x_attn)  ########### inner adapter
            x = x + x_adap + self.drop_path(x_fusion)  #########-------------------------adapter
        else:
            x = x + self.drop_path(x_attn)  # + self.drop_path(self.adap_t(xi_norm))  #########-------------------------adapter
        
        xi_attn, i_attn = self.attn(xi_norm, mask, True)  # norm + MHA
        if self.layer_num in [4,7,10]:
            xi_adap = self.iner_adap1(xi_attn)  ########### inner adapter
            xi = xi + xi_adap + self.drop_path(xi_fusion)  #########-------------------------adapter
        else:
            xi = xi + self.drop_path(xi_attn)  # + self.drop_path(self.adap_t(x_norm))  #########-------------------------adapter
        ...
        x_norm = self.norm2(x)
        xi_norm = self.norm2(xi)
        
        x_mlp = self.mlp(x_norm)
        if self.layer_num in [4,7,10]:
            x_side_adap = self.side_adap(x_mlp)
            x_mlp = x_mlp + x_side_adap
            x_adap = self.iner_adap2(x_mlp)  ########### inner adapter
            x_xi_fusion = torch.cat([x_norm, xi_norm], dim=-1)
            x_xi_fusion = self.adap2_t(x_xi_fusion)
            x_fusion, xi_fusion = torch.split(x_xi_fusion, x_norm.size(-1), dim=-1)
            x = x + x_adap + self.drop_path(x_fusion)
        else:
            x = x + self.drop_path(x_mlp) #+ self.drop_path(self.adap2_t(xi_norm))
        
        xi_mlp = self.mlp(xi_norm)
        if self.layer_num in [4,7,10]:
            xi_side_adap = self.side_adap(xi_mlp)
            xi_mlp = xi_mlp + xi_side_adap
            xi_adap = self.iner_adap2(xi_mlp)
            xi = xi + xi_adap + self.drop_path(xi_fusion)
        else:
            xi = xi + self.drop_path(xi_mlp) #+ self.drop_path(self.adap2_t(x_norm))
