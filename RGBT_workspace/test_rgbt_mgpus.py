...
from lib.test.tracker.mfja import MFJATrack
import lib.test.parameter.mfja as rgbt_mfja_adapter_params
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=300, debug=0, script_name='adapter'):
    ...
    if script_name == 'mfja':
        params = rgbt_mfja_adapter_params.parameters(yaml_name, epoch)
        mmtrack = MFJATrack(params)  # "GTOT" # dataset_name
        tracker = MFJA_RGBT(tracker=mmtrack)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--script_name', type=str, default='mfja', help='Name of tracking method(ostrack, adapter, ftuning, mfja).')
    ...
    if dataset_name == 'GTOT':
        ...
        seq_home = 'E:/dataset/RGB-T234' #'/hy-tmp/RGB-T234'  # '/media/xx/data1/dataset/RGB-T234'  E:\dataset\RGB-T234
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
        ...
        seq_home = 'E:/dataset/lasher/testingset'  # '/hy-tmp/lasher/testingset'  # '/media/xx/data1/dataset/lasher/testingset'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
    ...
    if args.mode == 'parallel':
        ...
        with open('seq_fps.txt', 'w') as f:
            fps_sum = 0
            for seq_fps in seq_fps_list:
                f.write(f'{seq_fps[0]}: {seq_fps[1]}\n')
                fps_sum+= seq_fps[1]
            f.write(f'average fps: {fps_sum/len(seq_fps_list)}')
            print(f'average fps: {fps_sum/len(seq_fps_list)}')
    print('----------------------------------')
